 ...
 * @author Valery
 */
public class TestDockTabPaneNEW  extends Application {
    @Override
    public void start(Stage primaryStage) throws Exception {
        TabPane primaryRoot = new TabPane();
        ...
        TargetContext ctx = DockRegistry.dockTarget(primaryRoot).getTargetContext();
        //((TabPaneContext)ctx).doDock(0,(Node)custom);
        Tab customTab = new Tab();
        customTab.setContent(custom);
        primaryRoot.getTabs().add(customTab);
        //primaryRoot.dock(custom);
        Tab tab1 = new Tab("tab1", new Button("Tab1 btn1"));
        primaryRoot.getTabs().add(tab1);
        Tab tab2 = new Tab("tab2", new Button("Tab2 btn2"));
        primaryRoot.getTabs().add(tab2);
        
        custom.setId("custom");
        DockNode custom1 = new DockNode();
        custom1.setTitle("CUSTOM 1");
        Tab customTab1 = new Tab();
        customTab1.setContent(custom1);
        primaryRoot.getTabs().add(customTab1);
        
        //((TabPaneContext)ctx).doDock(1,(Node)custom1);
        //primaryRoot.dock(custom1);
