...
public class TitleBarProperty<T extends Region> extends ObjectPropertyBase<T> {
    ...
    public void titlebarChanged(ObservableValue ov, Node oldValue, Node newValue) {
        if ( oldValue != null ) {
            ...
            if ( f != null ) {
                DockRegistry.dockable(f).getDockableContext().titleBarProperty().setActiveChoosedPseudoClass(false);
            }
        }
        if (newValue != null ) {
            ...
            if ( f != null ) {
                //DockRegistry.dockable(f).getDockableContext().titleBarProperty().setActiveChoosedPseudoClass(true);
            }
        }
    }
}
