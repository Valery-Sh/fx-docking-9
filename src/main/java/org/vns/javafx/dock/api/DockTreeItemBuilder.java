 ...
 * type {@code javafx.scene.control.TreeItem} and vice versa.
 *
 * Each class that inherits the {@link org.vns.javafx.dock.api.DockTargetController}} 
 * provides a class object that implements this interface. The object is 
 * accessed using the following  method of the class {@code DockTargetController}:
 * <pre>
...
public interface DockTreeItemBuilder {
     ...
     * </pre>
     * The {@code CLASS_NAME_ATTR} property is the string value of the fully specified 
     * object class name from the Scene Graph for which the TreeItem is built.
     * <p>
     * The {@code TAG_NAME_ATTR} property is the string value which may be used 
     * as an {@code xml} name of the tag corresponding to the Scene Graph object.
     * Usually this value is the same as the simple class name of the object, 
     * but the  {@code DockTreeItemBuilder} implementation can use any valid 
     * value for {@code xml tag}  names of {@code xml }documents.
     * </p>
     * <p>
     ...
     * {@link org.vns.javafx.dock.api.Dockable} object, 
     * then the value of  the {@code ISDOCKABLE_ATTR} property is set. 
     * This can be the value {@code "yes ", * "true" or "no ", "false "}. 
     * If the property value is not set, then the default value is {@code "no"}.
     * </p>
     * <p>
     * If an object from the {@code Scene Graph} is a 
     * {@link org.vns.javafx.dock.api.DockTarget} object, 
     * then the value of  the {@code ISDOCKTARGET_ATTR} property is set. 
     * This can be the value {@code "yes ", * "true" or "no ", "false "}. 
     * If the property value is not set, then the default value is {@code "no"}.
     * </p>
     * <p>
     *   Such an important property as {@code FIELD_NAME_ATTR} is set only 
     ...
     * interface implementation can set in {@code TreeItem} any additional 
     * properties of objects from the {@code Scene Graph}. Typically, 
     * these properties affect layout in one way or another, for example, 
     * {@code dividerPositions} from {@code SplitPane} or {@code ORIENTATION}.
     * </p>
     * 
     ...
     * of {@code null}. The method is implemented as {@code default}.
     * 
     * @return an object of type {@code javafx.scene.control.TreeItem}
     ...
     * 
     * @param obj the object for which a node of type {@code javafx.scene.control.TreeItem} is built
     * @return an object of type {@code javafx.scene.control.TreeItem}
     ...
     * object. The field name can be {@code null}. 
     * The work of the method is to set 
     * <pre>
