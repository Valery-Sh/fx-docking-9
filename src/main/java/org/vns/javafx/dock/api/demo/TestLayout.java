...
public class TestLayout extends Application {
    ...
    VBox rightPane = new VBox() {
        ...
        protected void layoutChildren() {
            ...
            if ( last != null) {
                System.err.println("layoutChildren: last Bounds = " + last.localToScreen(last.getLayoutBounds()));
                Platform.runLater(() -> {
                    System.err.println("RunLater: layoutChildren: last Bounds = " + last.localToParent(last.getLayoutBounds()));                    
                });
            }
        }
    };
    ...
    public void start(Stage stage) throws Exception {
        ...
        PopupWindow leftPopup = new PopupControl();
        
        leftPopup.getScene().setRoot(popupPane);
...
//        leftPopup.getScene().setRoot(leftPane);
        ...
        addButton.setOnAction(a -> {
            ...
            rightPane.getChildren().add(bp);
            //bp.setBottom(last);
        });
        Button showPopupBtn = new Button("Show Popup");
        
        leftPane.getChildren().add(showPopupBtn);
        showPopupBtn.setOnAction(a -> {
            popupPane.setStyle("-fx-border-color: red; -fx-border-width: 1; -fx-background-color: yellow");
        ...
        popupPane.setOnMouseReleased(a -> {
            last = new Button("Button" + counter++);
            //oBounds.bind(last.boundsInParentProperty());
            //rightPane.getChildren().add(last);
            bp.setBottom(last);
