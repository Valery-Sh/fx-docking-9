...
public class TestDoubleBinder extends Application {

    private final ObjectProperty<Color> sceneColorProperty =  new SimpleObjectProperty<>(Color.WHITE);

    @Override
    public void start(Stage primaryStage) {
        Button btn1 = new Button("change btn1Widh");
        //btn1.setPadding(new Insets(15,15,15,15));
        Button btn2 = new Button("change btn1.prefWidth");
        Button btn3 = new Button("show results");
        
        DoubleProperty btn1Width = new SimpleDoubleProperty(0);
        DoubleBinder dbb = new DoubleBinder(btn1Width,btn1.prefWidthProperty());
        dbb.change(btn1Width, (value) -> {
            return value * 2;
        });
        dbb.change(btn1.prefWidthProperty(), (value) -> {
            return value / 2;
        });
