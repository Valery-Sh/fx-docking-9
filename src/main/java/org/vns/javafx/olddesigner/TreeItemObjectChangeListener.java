...
public class TreeItemObjectChangeListener implements ChangeListener {
    ...
    public void changed(ObservableValue observable, Object oldValue, Object newValue) {
        NodeDescriptor nd = NodeDescriptorRegistry.getInstance().getDescriptor(treeItem.getObject());
        int idx = -1;
        ...
        for (int i = 0; i < treeItem.getChildren().size(); i++) {
            ...
            if (idx == it.getValue().getIndex()) {
                propTreeItem = it;
                break;
            }
            if (it.getValue().getIndex() >= insertPos && i < idx) {
                insertPos = i;
            }
        }
